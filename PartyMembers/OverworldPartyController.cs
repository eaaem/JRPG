...
public partial class OverworldPartyController : CharacterBody3D
{
   ...
   public Vector3 MovementTarget
   {
	...
	{ 
      ...
      partyMenuManager.PartySwap += OnSwap;
   ...
   /*public void OnClickFighter(Node camera, InputEvent @event, Vector3 position, Vector3 normal, long shapeidx)
   {
      ...
      {
         if (mouseButton.Pressed && isActive)
         {
            combatManager.ReceiveFighterSelection(this);
         }
      }
   }*/
   ...
   public void OnHoverOverPartyMember()
   {
      ...
      {
         combatManager.ProcessHoverOver(combatManager.GetFighterFromMember(assignedPartyMember));
      }
   }
   ...
   public void OnStopHoverOverPartyMember()
   {
      if (combatManager.IsInCombat)
      {
         combatManager.ProcessStopHoverOver(combatManager.GetFighterFromMember(assignedPartyMember));
      }
   }
