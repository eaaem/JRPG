...
public partial class DialogueManager : Node
{
  ...
  // int currentBranchLength;
   ...
   public bool cutsceneReadyForNextDialogue;

   public bool lockInput;

   //RayCast3D ray;

   //uint collisionMask = (uint)Mathf.Pow(2, 2);
	...
	public override void _Ready()
	{
      ...
      //ray = GetNode<RayCast3D>("DialogueRay");
      controller = GetNode<CharacterController>("/root/BaseNode/PartyMembers/Member1");
      playerModel = controller.GetNode<Node3D>("Model");
      camera = controller.GetNode<Camera3D>("CameraTarget/PlayerCamera");
      cutsceneManager = baseNode.GetNode<CutsceneManager>("CutsceneManager");
      shopMenuManager = baseNode.GetNode<ShopMenuManager>("UI/Shop/Background");
   ...
   public async void NextDialogue(int index)
   {
      ...
      if (currentObject.opensShop)
      {
         ...
         shopMenuManager.OpenShop(currentCollider.GetParent().GetNode<ShopItem>("ShopHolder"));
      ...
      if (currentDialogueList.isBranching)
      {
         ClearBranches();
         GenerateBranches();
      ...
      for (int i = 0; i < currentObject.content.Length; i++)
      {
         ...
         // Add BBcode tags in blocks; otherwise, they'll be entered in awkwardly a character at a time
   ...
   void GenerateBranches()
   {
      ...
      for (int i = 0; i < currentDialogueList.branchingDialogues.Length; i++)
      {
         ...
         GD.Print(currentBranchButton.Disabled);
      }
   }
   ...
   public void NextCutsceneDialogue()
   {
      ...
      {
         cutsceneManager.ProgressCutscene(currentIndex + 1);
      }
   ...
   public override void _Input(InputEvent @event)
   {
      if (@event.IsActionPressed("interact") && !lockInput)
      {
         ...
         {
            /*ray.TargetPosition = -playerModel.Basis.Z;
