      ...
      if (combatManager.CurrentAbility == resource)
      {
         ...
         for (int i = 0; i < combatManager.CurrentTarget.stacks.Count; i++)
         {
            if (combatManager.CurrentTarget.stacks[i].stackName == "Elemental Rot (Fire)")
            {
               ...
               combatManager.ApplyStatus(100, combatManager.CurrentTarget, StatusEffect.Burn, stackQuantity + 1, stackQuantity + 1);
               ...
               combatManager.RemoveStack(combatManager.CurrentTarget, "Elemental Rot (Fire)", stackQuantity);
            }
         }
