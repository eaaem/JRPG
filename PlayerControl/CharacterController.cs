...
public partial class CharacterController : CharacterBody3D
{
   ...
   public override void _Ready()
   {
     ...
     /*BoneAttachment3D ikAttachment = new BoneAttachment3D();
     ikAttachment.Name = "IKAttachment";
     skeleton.AddChild(ikAttachment);
     ikAttachment.BoneName = "hand.L";

     Node3D position = new Node3D();
     position.Name = "Pos";
     position.Position = new Vector3(0, 0.5f, 0);
     ikAttachment.AddChild(position);*/

    // weapon.Rotation = new Vector3(Mathf.DegToRad(-128f), Mathf.DegToRad(-11f), Mathf.DegToRad(76f));
   }
   ...
   public void OnHoverOverPartyMember()
   {
      if (combatManager.IsInCombat)
      {
         combatManager.ProcessHoverOver(combatManager.GetFighterFromMember(partyManager.Party[0]));
      }
   }

   public void OnStopHoverOverPartyMember()
   {
      if (combatManager.IsInCombat)
      {
         combatManager.ProcessStopHoverOver(combatManager.GetFighterFromMember(partyManager.Party[0]));
      }
   }
